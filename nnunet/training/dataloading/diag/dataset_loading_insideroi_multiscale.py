...
class DataLoader2DROIsMultiScaleFilename(DataLoader2D):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            matches = re.search(self.regex_pattern, selected_keys[j])
    ...
    def generate_single_roi(self):
        ...
        matches = re.search(self.regex_pattern, selected_key)
        file_name = matches.group("file")
        x = int(matches.group("x"))
        y = int(matches.group("y"))
        ...
        data = np.stack([case_all_data[:-1], self.sample_context(properties, x, y, file_name, (w, h))])
