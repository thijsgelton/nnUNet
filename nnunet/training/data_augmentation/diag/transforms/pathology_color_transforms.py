...
class HsvTransform(AbstractTransform):
    def __init__(self, h_lim=0.4, s_lim=0.4, v_lim=0.4, data_key="data", label_key="seg", p_per_sample=1):
        ...
        self.s_lim = s_lim
        self.v_lim = v_lim
        self.hsv_transform = A.ReplayCompose([HueSaturationValue(hue_shift_limit=self.h_lim, sat_shift_limit=self.s_lim,
        ...
        for b in range(data.shape[0]):
            if np.random.uniform() < self.p_per_sample:
                if data[b].ndim == 4:
                    ...
                    init = self.hsv_transform(image=d[0])
...
def combine_stains(stains, conv_matrix):
    stains = dtype.img_as_float(stains.astype('float64')).astype(
        'float32')  # stains are out of range [-1, 1] so dtype.img_as_float complains if not float64
...
class HedColorAugmenter():
    ...
    def __init__(self, haematoxylin_sigma_range, haematoxylin_bias_range, eosin_sigma_range, eosin_bias_range,
        ...
        # Save configuration.
        #
        self.__setsigmaranges(haematoxylin_sigma_range=haematoxylin_sigma_range, eosin_sigma_range=eosin_sigma_range,
                              dab_sigma_range=dab_sigma_range)
        self.__setbiasranges(haematoxylin_bias_range=haematoxylin_bias_range, eosin_bias_range=eosin_bias_range,
    ...
    def __setsigmaranges(self, haematoxylin_sigma_range, eosin_sigma_range, dab_sigma_range):
        ...
        if haematoxylin_sigma_range is not None:
            if len(haematoxylin_sigma_range) != 2 or haematoxylin_sigma_range[1] < haematoxylin_sigma_range[0] or \
                    haematoxylin_sigma_range[0] < -1.0 or 1.0 < haematoxylin_sigma_range[1]:
                raise Exception("InvalidHaematoxylinSigmaRangeError(haematoxylin_sigma_range)")

        if eosin_sigma_range is not None:
            if len(eosin_sigma_range) != 2 or eosin_sigma_range[1] < eosin_sigma_range[0] or eosin_sigma_range[
                0] < -1.0 or 1.0 < eosin_sigma_range[1]:
                raise Exception("InvalidEosinSigmaRangeError(eosin_sigma_range)")

        if dab_sigma_range is not None:
            if len(dab_sigma_range) != 2 or dab_sigma_range[1] < dab_sigma_range[0] or dab_sigma_range[
                0] < -1.0 or 1.0 < dab_sigma_range[1]:
    ...
    def __setbiasranges(self, haematoxylin_bias_range, eosin_bias_range, dab_bias_range):
        ...
        if haematoxylin_bias_range is not None:
            if len(haematoxylin_bias_range) != 2 or haematoxylin_bias_range[1] < haematoxylin_bias_range[0] or \
                    haematoxylin_bias_range[0] < -1.0 or 1.0 < haematoxylin_bias_range[1]:
                raise Exception("InvalidHaematoxylinBiasRangeError(haematoxylin_bias_range)")

        if eosin_bias_range is not None:
            if len(eosin_bias_range) != 2 or eosin_bias_range[1] < eosin_bias_range[0] or eosin_bias_range[
                0] < -1.0 or 1.0 < eosin_bias_range[1]:
                raise Exception("InvalidEosinBiasRangeError(eosin_bias_range)")

        if dab_bias_range is not None:
            if len(dab_bias_range) != 2 or dab_bias_range[1] < dab_bias_range[0] or dab_bias_range[0] < -1.0 or 1.0 < \
                    dab_bias_range[1]:
    ...
    def __setcutoffrange(self, cutoff_range):
        ...
        if cutoff_range is not None:
            if len(cutoff_range) != 2 or cutoff_range[1] < cutoff_range[0] or cutoff_range[0] < 0.0 or 1.0 < \
                    cutoff_range[1]:
    ...
    def transform(self, patch):
        ...
        if self.__cutoff_range[0] <= patch_mean <= self.__cutoff_range[1]:
            ...
            return np.clip(a=patch_rgb, a_min=0.0, a_max=1.0)
    ...
    def randomize(self):
        ...
        self.__sigmas = [
            ...
            for sigma_range in self.__sigma_ranges]
        self.__biases = [
            np.random.uniform(low=bias_range[0], high=bias_range[1], size=None) if bias_range is not None else 0.0 for
            bias_range in self.__bias_ranges]
